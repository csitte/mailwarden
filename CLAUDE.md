...
- **Kein Senden — by design.** Es gibt keine compose/reply/forward/send-Tools. Das ist die
  ...
  Checkpoint um `request()` des Auth-Clients, Allowlist der 17 tatsächlich genutzten Endpunkte —
  **das sind 15 Einträge in `ALLOWED`**, weil einer `modify|trash|untrash` in einem Ausdruck
  zusammenfasst; wer die Datei zählt, kommt auf 15, wer Endpunkte zählt, auf 17 (csitte kam bei der
  Verifikation seinerzeit auf 14 bei damals 13/15, und das kam aus genau dieser Doppeldeutigkeit —
  die Zahlen wandern also, die Doppeldeutigkeit bleibt; am 03.09. kamen `labels.patch` für
  Label-Farben und `history.list` für `what_changed` dazu. **Merksatz daraus: eine Zahl, die in zwei
  Dateien steht, wandert; wer einen Endpunkt hinzufügt, sucht beide.**) — plus
...
- `src/` — `auth.ts` (OAuth/Token/Accounts), `gmail.ts` (API-Wrapper), `tools.ts`
  ...
  `digest.ts`, `signals.ts` (Header-Signale), `authentication.ts` (SPF/DKIM/DMARC aus
  `Authentication-Results`), `unsubscribe.ts` (RFC 8058 + SSRF-Guard),
