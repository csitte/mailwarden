 ...
 * first measured on 15.08.2026 turned out to be specific to it: re-measured across both endpoints the
 * night after, the same query through `messages.list` in the same mailbox in the same minute returned
 * 19 hits with none stale, against 132 threads of which 114 were (`docs/measurements.json`,
 * endpoint-split-2026-08-16). Read a result here as a statement about the THREAD index, not about Gmail search at large.
 *
